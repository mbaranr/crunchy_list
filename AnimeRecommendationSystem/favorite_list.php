...
<?php
    ...
    // if we have selected add favorite

    if(isset($_POST['add_favorite'])){
        // fetch the anime name from the current session
        $anime = $_SESSION['anime'];
        $sql = "SELECT * FROM favorites WHERE user = '$username' AND anime = '$anime' ";
        $result = mysqli_query($conn, $sql);
        $queryResults = mysqli_num_rows($result);

        //if the anime is already in favorites don't do anything
        if($queryResults > 0){
            
        }else{

            // if it isn't, add it to the list with the respective username
    ...
    // if we have selected remove from favorites

    if(isset($_POST['rem_favorite'])){

        $anime = mysqli_real_escape_string($conn, $_SESSION['anime']);
        $sql = "SELECT * FROM favorites WHERE user = '$username' AND anime = '$anime' ";
        $result = mysqli_query($conn, $sql);
        $queryResults = mysqli_num_rows($result);

        //if the anime in the favorites list, remove it
        if($queryResults > 0){
...
<div class = "anime-container">
    ...
    // fetching all the animes from the user's favorite list
    ...
    while($row = mysqli_fetch_assoc($result)){
        ...
        echo "<a href='anime_info.php?title=".$row['anime']."'>";               //title
        echo "<img src='".$imgpath."' alt='Image' width='200' height='300'>";   //image
        echo "</a>";
        echo "</div>";
        echo "<h3><a href='anime_info.php?title=".$row['anime']."'>".$row['anime']."";  //reference to the info page
        echo "</a></h3>";
        echo "<p>";
